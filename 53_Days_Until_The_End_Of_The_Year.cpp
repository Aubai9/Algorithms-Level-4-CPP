...
short Days_Until_The_End_Of_The_Year(stDate date,short DaysInCurrentMonth)
{
    short TotalDays = DaysInCurrentMonth;
    for(short i = date.month + 1; i <= 12; i++)
        {
            TotalDays +=  Calc_Days_In_Month(i,date.year);
        }
    return TotalDays;
}
...
int main()
{
    ...
    short DaysUntilTheEndOfTheMonth = Days_Until_The_End_Of_The_Month(date);

    cout<<"Today is: "<<Day<<endl;

    (Days_Until_The_End_Of_The_Year(date,DaysUntilTheEndOfTheMonth) == 0) ? cout<<"Today is the end of the Year\n" : cout<<"Days until end of Year: "<<Days_Until_The_End_Of_The_Year(date,DaysUntilTheEndOfTheMonth);
