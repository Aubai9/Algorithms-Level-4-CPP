...
{
    if(Check_Last_Month_In_Year(date))
        {
            date.month = 1;
            date.year += 1;
        }
...
stDate Increase_Date_By_X_Month(stDate  date,short Added_Month)
{

    for(int i = 1; i <= Added_Month ; i++ )
        {
...
int main()
{
    ...
    Date_After_Addition = Increase_Date_By_X_Month(date,Added_Month);
